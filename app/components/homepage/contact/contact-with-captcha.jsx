...
function ContactWithCaptcha() {
  ...
  const checkRequired = () => {
    setError((prevError) => ({
      ...prevError,
      required: !(input.email && input.message && input.name),
    }));
  };
  ...
  const handleSendMail = async (e) => {
    ...
    const res = await axios.post(`${process.env.NEXT_PUBLIC_APP_URL}/api/google`, { token: captcha });
    setCaptcha(null);
        ...
        <div className="mt-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              onBlur={() => {
                checkRequired();
                setError((prevError) => ({ ...prevError, email: !isValidEmail(input.email) }));
              }}
            />
          ...
          <div className="flex flex-col items-center gap-2">
            {error.required && <p className="text-sm text-red-400">Email, Name, and Message are required!</p>}
            <button
